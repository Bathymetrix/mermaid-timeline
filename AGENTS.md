...
- Incremental pipeline outputs are now organized per float under the output root, and the per-float directory should use the full float serial when it can be discovered reliably from corpus layout or source content; fall back to the short stem ID only when no full serial is available.
